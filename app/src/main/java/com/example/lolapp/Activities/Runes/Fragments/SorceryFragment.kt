...
class SorceryFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_sorcery, container, false)
    ...
    private fun loadSorceryRunes() {
        CoroutineScope(Dispatchers.IO).launch {
            val db = DatabaseProvider.getDatabase(requireContext())
            val dao = db.runeDao()

            var runeEntity: RuneEntity? = dao.getRuneByKey("Sorcery")
            ...
            if (runeEntity == null) {
                try {
                    ...
                    val domination = runes.find { it.key.equals("Sorcery", ignoreCase = true) }
