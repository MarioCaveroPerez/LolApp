...
class ItemViewHolder(view: View, private val allItems: List<Item>) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(item: Item, activity: FragmentActivity, onItemClick: (String) -> Unit) {
        ...
        Picasso.get().load("https://ddragon.leagueoflegends.com/cdn/15.21.1/img/item/${item.image.full}").into(binding.imageChampion)
        binding.root.setOnClickListener { onItemClick(item.name) }
        binding.root.setOnClickListener {
            val fragment = ItemDetailBottomSheetFragment(item, allItems)
